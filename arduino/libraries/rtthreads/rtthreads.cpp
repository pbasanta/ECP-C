				...
				void (*error_handler)(int errno))
{
  ...
  base_th->ats_max=0;
  base_th->blocked_semaphore=NULL;
  return base_th;
}
...
//0=0k -1=not found
//Falla en el primer elemento
//Falla en el primer elemento
int thread_remove(THREAD * th)
{
  int returned=-1;
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
  THREAD*  th_previous=thread_list;
  THREAD*  th_index=thread_list;
  for(th_index=thread_list; th_index!=thread_main_inf; th_index=th_index->runq_next)
  { 
    if(th_index==th)
    {
      break;
    }
    th_previous=th_index;
  }
  
  if(thread_main_inf!=th_index) //Encontrado
  {
    if(th_index==thread_list)
    {
      thread_list=thread_list->runq_next; //quito de la cabeza
    }
    else
    {
      th_previous->runq_next=th->runq_next; //desenlazado
    }
    //Común borrar los dos mallocs
    returned=0;
    thread_free(th->stack);
    thread_free(th);
...
THREAD * thread_create(void (*entry)(void), uint8_t * stack, uint16_t stack_size, uint8_t priority, void * parm)
{
  ...
  *th->sp-- = 0x00; //For debuging purposes
  *th->sp-- = 0x11;
  *th->sp-- = 0x22;
  *th->sp-- = 0x23;
  th->aux1=0;
  ...
  *th->sp-- = (((uint16_t)entry) >> 8) & 0xff;
  ...
  //for(int i=0;i<35;i++){ //GC for(int i=0;i<33;i++){ //GC
  //  *th->sp-- =0x00;
  //}
  ...
  if(thread_list==NULL){
    THREAD* thmain=NULL;
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
      thmain=(THREAD*) malloc(sizeof(THREAD)); 
      thread_main_inf=thmain;
    }
    thmain->priority=SIMPLERTFRAMEWORK_NORM_PRIO;
    thmain->parm=NULL;
    thmain->max_cost_ticks= ((0)-2)/2;
    thmain->stack=(uint8_t *)RAMEND-300; //RAMEND refers to the end of the RAM memory
    thmain->stack_size=300;
    thmain->blocked_semaphore=NULL;
    thmain->current_cost_ticks=0;
    th->status=SIMPLERTFRAMEWORK_ACTIVE;
    thread_list=thmain;
    thmain->runq_next=NULL;
    thmain->ats_active=0;
    thmain->ats_pending=0;
    thmain->ats_max=0;
...
volatile uint8_t pxCurrentTCB_aux[2]={0,0};
...
void do_switch() __attribute__((naked));
void do_switch(){  
  asm volatile("cli");
  ...
  asm volatile("push r0");
  asm volatile("ldi r16, 0xAB"); //GC
  asm volatile("push r16");//GC
  asm volatile("push r16");//GC
...
void thread_start(struct thread_t * th){
  ...
  next_thread=th;
  do_switch();
  //if pending handlers, activate them.
  if(current_thread->ats_active==1)
  {
    
    while(current_thread->ats_pending!=0)
    {
      current_thread->ats_pending--;
...
void ats_wrapper() __attribute__ ((naked));
void ats_wrapper(){
    while(1)
    {
      current_thread->aux2++;
...
{
  //current_thread->ats_max=max;
}
int32_t thread_ATS_get_maximum()
{
  //return current_thread->ats_max;
  return 0;
}
/*
 * No tested yet
...
{
   QUEUE* all= (QUEUE*) thread_malloc(sizeof(void*)*howmany+sizeof(QUEUE));
   all->howmany=howmany;
   all->stored=0;
   all->read_index=0;
   all->write_index=0;
...
void* thread_queue_nb_get_element(QUEUE* queue)
{
	...
	result=NULL;
	 ...
	 //Reading 
	 result=queue_l->elements[queue_l->read_index];
	  //Updating
	 queue_l->stored--;
	 queue_l->read_index++;
	 
	 if(queue_l->read_index==0)
	 {
	   queue_l->read_index=queue_l->howmany;
	 }
	 //queue_l->read_index=queue_l->read_index % queue_l->howmany;
	 // pbv
...
int32_t   thread_queue_nb_set_element(QUEUE* queue, void* element)
{
      int32_t  result=0;
      ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
      QUEUE_L* queue_l=(QUEUE_L*) queue;
      if( queue_l->stored== queue_l->howmany)
	result=-1;
      else{
	 //Writing
	 queue_l->elements[queue_l->write_index]=element;
	  //Updating
	 queue_l->stored++;
	 queue_l->write_index++;
	 
	 if(queue_l->write_index==queue_l->howmany)
	 {
	   queue_l->write_index=0;
	 }
	// queue_l->write_index=queue_l->write_index % queue_l->howmany;
	//pbv
...
int32_t  thread_queue_stored(QUEUE* queue)
{
    uint32_t  result=0;
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
      result=queue->stored; 
...
int32_t  thread_queue_size(QUEUE* queue)
{
    int32_t  result=0;
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
      result=queue->howmany; 
    }
 return result; 
}
/**********************************************************************/
/* Simple semaphore simplementation
 */
...
SEMAPHORE* thread_semaphore_create(int initialization)
{
  ...
  SEMAPHORE* sem_aux=NULL;
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
    sem_aux=(SEMAPHORE*) thread_malloc(sizeof(SEMAPHORE));
    sem_aux->value=initialization;
  }
  return sem_aux;
}
/*Create sem*/
void thread_semaphore_P(SEMAPHORE* sem)
{
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
  {
     ...
     if(sem->value<=0)
     {
       current_thread->blocked_semaphore=sem;
       current_thread->status=SIMPLERTFRAMEWORK_SEM_BLOCKED;
       thread_yield();
     }
     else
     {
...
void thread_semaphore_V(SEMAPHORE* sem)
{
    if(sem ==NULL)
      return;
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
    //for all threads search max priority less 
    THREAD* th_candidate=NULL;
    for(THREAD* th=thread_list; th!=thread_main_inf; th=th->runq_next)
      {
      if((th->blocked_semaphore==sem))
      {
	if (th_candidate==NULL)
	{
	  th_candidate=th;
	}else
	{
	  if(th_candidate->priority<th->priority)
	  {
	    th_candidate=th;
	  }
	}
    ...
    //UPDATE
    if (th_candidate!=NULL){
      th_candidate->status=SIMPLERTFRAMEWORK_ACTIVE;
      th_candidate->blocked_semaphore=NULL;
      thread_yield();
    }
    else
    {
...
{ //Clean all threads awaiting for this lock
  if(sem ==NULL)
    return;
  
  
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
    //Borrar todos las referencias al semaphoro
    
    for(THREAD* th=thread_list;th!=thread_main_inf;th=th->runq_next)
      if((th->status==SIMPLERTFRAMEWORK_SEM_BLOCKED)&&(th->blocked_semaphore==sem))
      {	 
	th->status=SIMPLERTFRAMEWORK_ACTIVE;
        th->blocked_semaphore=NULL;
